...
void print_piece_chessboard(int piece)
{
    for (int rank = 0; rank < DIMENSION; rank++)
    {
        ...
        {
            int square = rank * DIMENSION + file;
            std::cout << (get_bit(piece_occupancies[piece], square)
                              ? ASCII_PIECES[piece]
                              : '.')
                      << ' ';
        }
    ...
    std::cout << "\n    a b c d e f g h\n";
    std::cout << "\n decimal value = " << std::dec << piece_occupancies[piece] << '\n';
}
...
void print_chessboard()
{
    for (int rank = 0; rank < DIMENSION; rank++)
    {
        ...
        for (int file = 0; file < DIMENSION; file++)
        {
            ...
            {
                if (get_bit(piece_occupancies[piece], square))
                    ascii = ASCII_PIECES[piece];
            }
    ...
    std::cout << "\n    a b c d e f g h\n";
    std::cout << "\n decimal value = " << std::dec << (side_occupancies[WHITE] | side_occupancies[BLACK]) << '\n';
}
